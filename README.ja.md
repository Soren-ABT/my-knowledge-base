...
│   │   ├── CoreManager.astro            # 集中イベント委任
│   │   ├── MusicPlayer.astro            # 音楽プレーヤーシェル
│   │   └── ...                          # PostCard、Pagination、Search など
│   ├── config/                          # サイト設定（9 ファイル）
...
│   ├── pages/                           # ルートページ（13 ファイル）
│   ├── plugins/                         # Remark/Rehype プラグイン（12 ファイル）
│   ├── scripts/                         # TypeScript モジュール（18 ファイル）
│   │   ├── archive/                     # アーカイブロジック（river-math、drag、tabs、filter、observer）
│   │   ├── music-player/                # 音楽プレーヤー UI、プログレスバー、スペクトラム
│   │   ├── music/                       # 共有音楽 UI（volume-slider）
│   │   ├── client/                      # クライアントアプリロジック
│   │   ├── core-manager.ts              # 設定、壁紙、目次、リンク先読み
│   │   ├── welcome-wave.ts              # テーマ切り替え波アニメーション
│   │   └── typewriter.ts                # タイプライターエフェクト
│   ├── styles/                          # CSS デザインシステム
│   │   ├── archive/                     # アーカイブページスタイル（tabs、timeline、river）
│   │   ├── components/                  # 再利用可能コンポーネントスタイル
│   │   ├── music-player/                # 音楽プレーヤースタイル（core、panel、detail、lightbox）
│   │   ├── panels/                      # パネルスタイル（lightbox、search、settings、toc）
│   │   └── pages/                       # ページ固有スタイル（gallery、home）
...
実行時（TypeScript モジュール（src/scripts/）+ クライアントスクリプト（public/js/））：
  core-manager.ts: 設定パネル、壁紙カルーセル、浮動目次、リンク先読み、
                   テーマ遷移ガード、イベント委任

  app.js: 検索モーダル、ライトボックス、トップへ戻る、プログレスバー、
          リンクプレビュー、キーボードショートカット、ナビバースクロール

  music-player.js: Web Audio API DSP チェーン、HTML5 Audio 要素、
                   状態管理、パブリッシュ-サブスクライブブロードキャスト

  music-player/ui-core.ts: FAB、コンパクトパネル、ライブラリオーバーレイ、
                           没入再生、音量スライダー、プログレスバー、スペクトラム

  welcome-wave.ts: テーマ切り替え時の Canvas 波アニメーション
